...
//  在最大速度条件下，达到目标位置的所需要叠加的速度 如动物到食物
//------------------------------------------------------------------------
...
//  在最大速度条件下，达到目标位置的所需要叠加的速度 如猎物与捕猎者
//------------------------------------------------------------------------
...
Vector2D SteeringBehavior::Pursuit(const Vehicle* evader)
{
	...
	//如果逃避者在前面，且面对着智能体,智能体就直接向逃避者当前的位置运动
	...
	if ((ToEvader.Dot(m_pVehicle->Heading()) > 0) &&
		(RelativeHeading < -0.95))  //acos(0.95)=18 degs 夹角小于18
	{
...
Vector2D SteeringBehavior::ObstacleAvoidance(const std::vector<BaseGameEntity*>& obstacles)
{
	...
	while (curOb != obstacles.end())
	{
		...
		if ((*curOb)->IsTagged())
		{
			...
			//如果这个障碍物的x轴为负，那么肯定在之呢个体的后面，可以被忽略
			if (LocalPos.x >= 0)
			{
				//if the distance from the x axis to the object's position is less
				//than its radius + half the width of the detection box then there
				//is a potential intersection.
				//如果障碍物在局部控件中的y坐标的绝对值，小于它的半径+检查盒宽度的一半，那么可能相交
	...
	const Vehicle* AgentB)
{
	...
	//首先计算在未来时间T时，两个智能体的位置
	//设T近似与为来交通体以最大速度到达中点的时间
	...
	//现在有时间T,我们假设只能提A和B继续径直前行，推断他们的位置
